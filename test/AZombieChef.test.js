...
const ZombieChef = artifacts.require('ZombieChef');
const MockERC20 = artifacts.require('MockERC20');

contract('ZombieChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.sushi = await SushiToken.new({ from: alice });
    });

    it('should set correct state variables', async () => {
        this.chef = await ZombieChef.new(this.sushi.address, '1000', '0', '1000', { from: alice });
    ...
    it('should set bonus end block', async () => {
        this.chef = await ZombieChef.new(this.sushi.address, '1000', '0', '1000', { from: alice });
        assert.equal((await this.chef.bonusEndBlock()).valueOf(), '1000');
        await this.chef.setBonusEndBlock('10', { from: alice })
        assert.equal((await this.chef.bonusEndBlock()).valueOf(), '10');
        await this.chef.setBonusEndBlock('10000', { from: alice })
        assert.equal((await this.chef.bonusEndBlock()).valueOf(), '10000');
        await expectRevert.unspecified(
            this.chef.setBonusEndBlock('10', { from: bob })
        );
        await expectRevert.unspecified(
            this.chef.setBonusEndBlock('10', { from: carol })
        );
    });

    // rotten swap does not have a dev fund
    // it('should allow dev and only dev to update dev', async () => {
    //     this.chef = await ZombieChef.new(this.sushi.address, dev, '1000', '0', '1000', { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await ZombieChef.new(this.sushi.address, '100', '100', '1000', { from: alice });
        ...
        it('should give out SUSHIs only after farming time', async () => {
            ...
            assert.equal((await this.sushi.totalSupply()).valueOf(), '5000');
        });

        it('should not distribute SUSHIs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.chef = await ZombieChef.new(this.sushi.address, '100', '200', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            ...
            assert.equal((await this.sushi.totalSupply()).valueOf(), '10000');
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            this.chef = await ZombieChef.new(this.sushi.address, '100', '300', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            //   ZombieChef should have the remaining: 10000 - 5666 = 4334
            ...
            assert.equal((await this.sushi.totalSupply()).valueOf(), '20000');
            assert.equal((await this.sushi.balanceOf(alice)).valueOf(), '5666');
            ...
            assert.equal((await this.sushi.totalSupply()).valueOf(), '50000');
            assert.equal((await this.sushi.balanceOf(dev)).valueOf(), '0');
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await ZombieChef.new(this.sushi.address, '100', '400', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            this.chef = await ZombieChef.new(this.sushi.address, '100', '500', '600', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
