...
// skipping because we're not using it yet
contract.skip('Migrator', ([alice, bob, dev, minter]) => {
    beforeEach(async () => {
        this.factory1 = await UniswapV2Factory.new(alice, { from: alice });
        this.factory2 = await UniswapV2Factory.new(alice, { from: alice });
        this.sushi = await RottenToken.new({ from: alice });
        ...
        this.chef = await ZombieChef.new(this.sushi.address, '1000', '0', '100000', { from: alice });
